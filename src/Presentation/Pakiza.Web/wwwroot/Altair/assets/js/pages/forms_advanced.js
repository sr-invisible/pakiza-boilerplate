$(function() {
    ...
    // advanced selects with Values
    altair_form_adv.rkAdvSelects();
    // masked_inputs
...
altair_form_adv = {
    ...
    rkAdvSelects: function(oSelector = '.rkAdvSelects', oNum = 1, mOpt = null) {
		if($(oSelector).length) {
			$(oSelector).selectize({
				plugins: {
					'remove_button': {
				...
				render: {
					option: function(data, escape) {
						return  '<div class="option"><span class="title">' + escape(data.title) + '</span></div>';
					},
    ...
    date_range: function() {
        ...
        var start_date = UIkit.datepicker($dp_start, UIkit.Utils.options($dp_start.attr("data-uk-datepicker")));

        var end_date = UIkit.datepicker($dp_end, UIkit.Utils.options($dp_end.attr("data-uk-datepicker")));
