...
(function (jQuery) {
    ...
    var Chart = function(div, opts) {
        ...
        function render() {
            ...
            addHzHeader(slideDiv, dates, opts.cellWidth, opts.showWeekends, opts.hDays);
            addGrid(slideDiv, opts.data, dates, opts.cellWidth, opts.showWeekends, opts.hDays);
        ...
        function addHzHeader(div, dates, cellWidth, showWeekends, hDays) {
            ...
            for (var y in dates) {
                for (var m in dates[y]) {
                    ...
                    for (var d in dates[y][m]) {
                        ...
                        if (DateUtils.isWeekend(dates[y][m][d], hDays) && showWeekends) {
        ...
        function addGrid(div, data, dates, cellWidth, showWeekends, hDays) {
                ...
                for (var j = 0; j < data[i].series.length; j++) {
                    ...
                    if(data[i].series[j].sub_series) {
                        ...
                        $.each(sub_series, function($key,$value) {
							...
							var tooltip_title = $value.myTooltip ? $value.myTooltip : $sub_series_name + user_name;
                        ...
                        var user_name = series.user_name ? ' (' + series.user_name + ')' : '';
						var tooltip_title = series.myTooltip ? series.myTooltip : $series_name + user_name;
                        var block = jQuery("<div>", {
                            "class": "ganttview-block",
                            "title": tooltip_title,
                            "css": {
    ...
    var Behavior = function (div, opts) {
        ...
        function apply() {
            ...
            if (opts.behavior.rbclickable) { 
                bindBlockRBClick(div, opts.behavior.onRBlClick);
            }
        ...
        function bindBlockRBClick(div, callback) {
            jQuery("div.ganttview-block", div).on("mousedown", function (e) {
				if (e.which == 3)
    ...
    var DateUtils = {
        ...
        isWeekend: function (date, hDays) {
			if(hDays.length > 0) { return jQuery.inArray(moment(new Date(date)).format('MM/DD/YYYY'), hDays) !== -1; }
			else { return (date.getDay() + 1) % 6 == 0; }
            ...
            for (var i = 0; i < data.length; i++) {
                for (var j = 0; j < data[i].series.length; j++) {
					if(data[i].series[j].sub_series) {
						...
						var k = 0;
                        $.each(sub_series, function($key,$value) {
							var start = Date.parse($value.start);
							var end = Date.parse($value.end);
